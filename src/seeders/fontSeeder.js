...
const categories = ['Sans Serif', 'Serif', 'Display', 'Handwriting', 'Monospace', 'Calligraphy'];
const styles = ['Regular', 'Bold', 'Italic', 'Light', 'Medium', 'Black', 'Thin'];
const uses = ['Website', 'Print', 'Branding', 'Logo', 'Advertising', 'Packaging', 'Editorial'];
const tags = ['modern', 'classic', 'elegant', 'vintage', 'decorative', 'minimalist', 'professional', 'playful', 'geometric', 'organic', 'retro', 'clean', 'bold', 'script', 'grunge'];

const generateRandomFont = (index) => {
  // Generate random name
  const fontNames = [
    'Nova', 'Stellar', 'Quantum', 'Echo', 'Vertex', 'Prism', 'Flux', 'Zenith', 
    'Atlas', 'Nexus', 'Cipher', 'Vector', 'Pulse', 'Orbit', 'Matrix'
  ];
  const fontVariants = [
    'Pro', 'Sans', 'Display', 'Text', 'Neue', 'Light', 'Modern', 'Classic'
  ];
...
const seedFonts = async () => {
  try {
    ...
    // Generate 100 fonts
    const fonts = Array.from({ length: 100 }, (_, index) => generateRandomFont(index));
